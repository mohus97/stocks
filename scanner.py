...
class IGDemoExecutor:
    ...
    def execute_signal(self, sig, source='5m'):
        ...
        zone_buffer = max(0.05 * original_risk_distance, abs(offer - bid) * 2.0)
        if not (float(sig.entry_low) - zone_buffer <= ig_entry <= float(sig.entry_high) + zone_buffer):
        ...
        if not any(str(c.get('code') or '').upper() == 'GBP' for c in currencies):
            telegram_notify(f'⚠️ IG DEMO AUTO: no GBP deal currency for {sig.label}; skipped rather than guess risk conversion.')
            return False, 'no GBP currency'
        ...
        risk_per_size_gbp = stop_distance * scaling * pip_value

...
def data_is_fresh(df, item, cfg):
    """Avoid alerts from stale last candles when a market is closed."""
    ...
    max_age = float(cfg.get('scanner', {}).get('max_candle_age_minutes', 20))
...
def score_signal(df, item, cfg):
    ...
    elif market_type in ('forex', 'index'):
        long_score += 0.5  # volume often unavailable on these Yahoo feeds
...
def format_signal(s: Signal):
    ...
    cfd_note = ''
    if s.market_type in ('forex', 'index') or s.side == 'SHORT':
    ...
    for item in cfg['watchlist']:
        ...
        try:
            ...
            if not data_is_fresh(df, item, cfg):
                print(f'[{datetime.now().strftime("%H:%M:%S")}] {symbol}: market/data stale; skip')
                continue

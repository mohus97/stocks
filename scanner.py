...
def main():
    ...
    # During a Railway redeploy the old and new containers can overlap briefly.
    # Skip Twelve Data on this immediate startup pass so both containers cannot
    # spend the same 8-credit/minute allowance at once.
    scan_once(cfg, include_twelvedata=False)
    while True:
        try:
            wait = seconds_to_next_5m()
            print(f'Next scan in {int(wait)}s...')
            time.sleep(wait)
            scan_once(cfg, include_twelvedata=True)
